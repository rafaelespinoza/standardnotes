...
// WriteHeader writes HTTP status code. If WriteHeader is not called explicitly,
...
// Write writes the data to the connection as part of an HTTP reply. If
...
// CloseNotify calls CloseNotify. It panics unless lw's ResponseWriter
			...
			return func(w http.ResponseWriter, r *http.Request) {
				...
				defer func() {
					if err := recover(); err != nil {
						...
						log.Printf(
							" %srecovering from panic: %+v\nStack Trace:\n %s%s",
							ansi.Red, err, trace[:n], ansi.Reset,
						)
				...
				log.Printf(
					"%s %d %s[%s%s%s] %q %v %d\n",
					color, code, ansi.Reset, color, r.Method,
					ansi.Reset, r.URL, time.Since(t1), lw.Size(),
				)
			}
		}
...
// HandlePanic handles graceful panic by redirecting to friendly error page or
// rendering a friendly error page.  trace passed just in case you want rendered
// to developer when not running in production
